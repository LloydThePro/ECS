...
int main(){
    ...
    vectorArray* verArr = ECS::getProperType<vectorArray>(EntityCreator::getComponent(obj, vectorArray::id));
   

    verArr->arrayObj.push_back({ 5,4 });
    std::cout << verArr->arrayObj[0].x;

    



    /*void* someMemory = (void*)new char[sizeof(vectorArray)];
    vectorArray* vecptr = (vectorArray*)someMemory;
    std::cout << vecptr << '\n';
    vecptr = new(vecptr) vectorArray;
    std::cout << vecptr << '\n';
    vecptr->arrayObj.push_back({ 2,2 });
    vecptr->arrayObj.push_back({ 3,2 });
    std::cout << vecptr->arrayObj[1].x;*/
